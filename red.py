...
def initialize(bot_class=Bot, formatter_class=Formatter):
    ...
    async def on_ready():
        ...
        # await bot.get_command("populate").callback()
        # await bot.get_command("elo registerserver").callback()
...
def load_cogs(bot):
    defaults = ("alias", "audio", "customcom", "downloader", "economy",
                "general", "image", "mod", "streams", "trivia", "elo")

    defaults = ("alias", "customcom", "mod", "elo")
